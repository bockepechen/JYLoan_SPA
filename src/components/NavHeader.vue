<template>
  <header class="header">
    <symbol id="icon-cart" viewBox="0 0 38 32">
      <title>cart</title>
      <path class="path1" d="M37.759 0h-4.133c-0.733 0.004-1.337 0.549-1.434 1.255l-0.546 4.342c-0.081 0.484-0.496 0.849-0.997 0.849-0.005 0-0.009-0-0.014-0h-27.604c-0.003 0-0.007-0-0.011-0-1.674 0-3.031 1.357-3.031 3.031 0 0.34 0.056 0.666 0.159 0.971l2.52 8.062c0.385 1.194 1.486 2.043 2.785 2.043 0.126 0 0.25-0.008 0.372-0.023l22.983 0.002c0.515 0.131 0.626 0.768 0.626 1.283 0.005 0.044 0.009 0.095 0.009 0.146 0 0.501-0.294 0.933-0.718 1.134l-22.439 0.003c-0.354 0-0.642 0.287-0.642 0.642s0.287 0.642 0.642 0.642h22.745l0.131-0.071c0.919-0.392 1.551-1.287 1.551-2.33 0-0.058-0.002-0.116-0.006-0.173 0.021-0.108 0.033-0.24 0.033-0.376 0-1.072-0.732-1.973-1.724-2.23l-23.357-0.004c-0.063 0.008-0.135 0.013-0.209 0.013-0.719 0-1.332-0.455-1.566-1.093l-2.53-8.095c-0.048-0.154-0.076-0.332-0.076-0.515 0-0.973 0.782-1.764 1.752-1.778h27.657c1.159-0.004 2.112-0.883 2.232-2.011l0.547-4.345c0.010-0.083 0.078-0.147 0.161-0.152l4.133-0c0.354 0 0.642-0.287 0.642-0.642s-0.287-0.642-0.642-0.642z"></path>
      <path class="path2" d="M31.323 9.69c-0.022-0.003-0.048-0.004-0.074-0.004-0.328 0-0.598 0.248-0.633 0.567l-0.809 7.268c-0.003 0.022-0.004 0.048-0.004 0.074 0 0.328 0.248 0.598 0.567 0.633l0.074 0c0.001 0 0.003 0 0.004 0 0.327 0 0.596-0.246 0.632-0.563l0.809-7.268c0.003-0.022 0.004-0.048 0.004-0.074 0-0.328-0.248-0.598-0.567-0.633z"></path>
      <path class="path3" d="M27.514 25.594c-1.769 0-3.203 1.434-3.203 3.203s1.434 3.203 3.203 3.203c1.769 0 3.203-1.434 3.203-3.203s-1.434-3.203-3.203-3.203zM27.514 30.717c-1.060 0-1.92-0.86-1.92-1.92s0.86-1.92 1.92-1.92c1.060 0 1.92 0.86 1.92 1.92s-0.86 1.92-1.92 1.92z"></path>
      <path class="path4" d="M9.599 25.594c-1.769 0-3.203 1.434-3.203 3.203s1.434 3.203 3.203 3.203c1.769 0 3.203-1.434 3.203-3.203s-1.434-3.203-3.203-3.203zM9.599 30.717c-1.060 0-1.92-0.86-1.92-1.92s0.86-1.92 1.92-1.92c1.060 0 1.92 0.86 1.92 1.92s-0.86 1.92-1.92 1.92z"></path>
    </symbol>
    <symbol id="icon-arrow-short" viewBox="0 0 25 32">
      <title>arrow-short</title>
      <path class="path1" d="M24.487 18.922l-1.948-1.948-8.904 8.904v-25.878h-2.783v25.878l-8.904-8.904-1.948 1.948 12.243 12.243z"></path>
    </symbol>
    <symbol id="icon-status-ok" viewBox="0 0 32 32">
      <title>status-ok</title>
      <path class="path1" d="M22.361 10.903l-9.71 9.063-2.998-2.998c-0.208-0.209-0.546-0.209-0.754 0s-0.208 0.546 0 0.754l3.363 3.363c0.104 0.104 0.241 0.156 0.377 0.156 0.131 0 0.261-0.048 0.364-0.143l10.087-9.414c0.215-0.201 0.227-0.539 0.026-0.754s-0.539-0.226-0.754-0.026z"></path>
      <path class="path2" d="M16 30.933c-8.234 0-14.933-6.699-14.933-14.933s6.699-14.933 14.933-14.933c8.234 0 14.933 6.699 14.933 14.933s-6.699 14.933-14.933 14.933zM16 0c-8.822 0-16 7.178-16 16 0 8.823 7.178 16 16 16s16-7.177 16-16c0-8.822-7.178-16-16-16z"></path>
    </symbol>
    <div class="navbar">      
      <div class="navbar-left-container">
        <a href="/">
          <img class="navbar-brand-logo" src="static/logo.png"></a>
      </div>
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          <!--<a href="/" class="navbar-link">我的账户</a>-->
          <span class="navbar-link"></span>
          <span v-if="nickName">{{nickName}}</span>
          <a href="javascript:void(0)" class="navbar-link" @click="clickLogin()" v-if="!nickName">Login</a>
          <a href="javascript:void(0)" class="navbar-link" @click="logOut()" v-if="nickName">Logout</a>
          <div class="navbar-cart-container">
            <span class="navbar-cart-count">{{cartNum}}</span>
            <a class="navbar-link navbar-cart-link" href="/#/cart">
              <svg class="navbar-cart-logo">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-cart"></use>
              </svg>
            </a>
          </div>
        </div>
      </div>
      <!--登录模态层-->
      <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':loginModal}">
        <div class="md-modal-inner">
          <div class="md-top">
            <div class="md-title">Login</div>
            <button class="md-close" @click="loginModal=false">Close</button>
          </div>
          <div class="md-content">
            <div class="confirm-tips">
              <div class="error-wrap">
                <span class="error error-show" v-show="errorTip">用户名或者密码错误</span>
              </div>
              <ul>
                <li class="regi_form_input">
                  <i class="icon IconPeople"></i>
                  <input type="text" tabindex="1" name="loginname" v-model="userName" placeholder="User Name" data-type="loginname" class="regi_login_input regi_login_input_left">
                </li>
                <li class="regi_form_input noMargin">
                  <i class="icon IconPwd"></i>
                  <input type="password" tabindex="2" name="password" v-model="userPwd" placeholder="Password" @keyup.13="login(userName, userPwd)" class="regi_login_input regi_login_input_left login-input-no input_text">
                </li>
              </ul>
            </div>
            <div class="login-wrap">
              <a href="javascript:;" class="btn-login" @click="login(userName, userPwd)">登  录</a>
            </div>
            <div class="login-wrap">
              <a href="javascript:;" class="btn-register" @click="clickRegister()">注  册</a>
            </div>
          </div>
        </div>
      </div>
      <!--注册模态层-->
      <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':registerModal}">
        <div class="md-modal-inner">
          <div class="md-top">
            <div class="md-title">Login</div>
            <button class="md-close" @click="registerModal=false">Close</button>
          </div>
          <div class="md-content">
            <div class="confirm-tips">
              <div class="error-wrap">
                <span class="error error-show" v-show="errorTip_reg">{{errorTip_reg}}</span>
              </div>
              <ul>
                <li class="regi_form_input">
                  <i class="icon IconPeople"></i>
                  <input type="text" tabindex="1" name="loginname" v-model="userNm_reg" placeholder="User Name" data-type="loginname" class="regi_login_input regi_login_input_left">
                </li>
                <li class="regi_form_input noMargin">
                  <i class="icon IconPwd"></i>
                  <input type="password" tabindex="2" name="password" v-model="userPwd_reg" placeholder="Password" class="regi_login_input regi_login_input_left login-input-no input_text">
                </li>
                <li class="regi_form_input noMargin">
                  <i class="icon IconPwd"></i>
                  <input type="password" tabindex="2" name="password-comfirm" v-model="userPwdCfm_reg" placeholder="Password-Confirm" class="regi_login_input regi_login_input_left login-input-no input_text">
                </li>
              </ul>
            </div>
            <div class="login-wrap">
              <a href="javascript:;" class="btn-login" @click="register()">注  册</a>
            </div>
          </div>
        </div>
      </div>
      <div class="md-overlay" v-if="loginModal||registerModal" @click="loginModal=false;registerModal=false;"></div>
    </div>
  </header>
</template>

<script>
import './../assets/css/login.css'
import axios from 'axios'
export default {
  data() {
    return {
      userName:'',
      userPwd:'',
      errorTip:false,
      loginModal:false,
      registerModal:false,
      userNm_reg:'',
      userPwd_reg:'',
      userPwdCfm_reg:'',
      errorTip_reg:''
    }
  },
  computed:{
    nickName() {
      return this.$store.state.nickName;
    },
    cartNum() {
      if (this.$store.state.cartNum == 0) {
        return '';
      } else {
        return this.$store.state.cartNum;
      }
    }
  },
  mounted() {
    // 验证登录状态，如果cookie中数据有效，不必重新登录
    this.checkLogin();
  },
  methods: {
    // 点击登录按钮
    clickLogin() {
      this.clearInputProp();
      this.loginModal=true;
      this.errorTip=false;
    },
    // 点击注册按钮
    clickRegister() {
      this.clearInputProp();
      this.loginModal = false;
      this.registerModal = true;
    },
    // 注册提交
    register() {
      // 校验两次输入密码是否一致
      if (this.userPwd_reg !== this.userPwdCfm_reg) {
        this.errorTip_reg = '两次密码不一致';  // 设置错误提示信息
        return;
      } else {
        this.errorTip_reg = '';  // 清空错误提示信息
      }
      // 提交后台服务
      axios.post('/users/register',{
        userNm_reg:this.userNm_reg,
        userPwd_reg:this.userPwd_reg
      }).then((res) => {
        let recData = res.data;
        if (recData.status == '200') {
          this.errorTip_reg = '注册成功';
          // 注册成功后，直接登录
          this.login(this.userNm_reg, this.userPwd_reg);
        } else {
          this.errorTip_reg = '注册失败，请联系管理员。';
        }
      });
    },
    // 登录提交
    login(userName_param, userPwd_param) {
      axios.post('/users/login', {
        userName:userName_param,
        userPwd:userPwd_param
      }).then((res)=>{
        let recData = res.data;
        if (recData.status == '200') {
          this.errorTip = false;
          this.loginModal = false;
          this.registerModal = false;
          this.errorTip_reg = false;
          // 设置Vuex中状态值
          this.$store.commit('updateNickName', "用户 ".concat(recData.result.userName));
          this.getCartNum();
        } else {
          this.errorTip = true;
        }
      });
    },
    // 登出提交
    logOut() {
      axios.post('/users/logout').then((res)=>{
        let recData = res.data;
        if(recData.status == '200') {
          // 清空Vuex中的状态值
          this.$store.commit('updateNickName', '');
          this.$store.commit('resetCartNum', 0);
        }
      });
    },
    // 验证登录状态
    checkLogin() {
      axios.get('/users/checkLogin').then((res)=>{
        let recData = res.data;
        if (recData.status == '200') {
          // 设置Vuex中状态值
          this.$store.commit('updateNickName', 'Vuex '.concat(recData.result.userName));
          this.getCartNum();
        }
      });
    },
    // 获取购物车内商品数量
    getCartNum() {
      axios.get('/users/getCartNum').then((res)=> {
        let recData = res.data;
        if (recData.status == '200') {
          //设置Vuex中状态
          this.$store.commit('resetCartNum', recData.result);
        }
      });
    },
    // 清除录入项
    clearInputProp() {
      this.userName = '';
      this.userNm_reg = '';
      this.userPwd = '';
      this.userPwd_reg = '';
      this.userPwdCfm_reg = '';
      this.errorTip = '';
      this.errorTip_reg = '';
    }
  }
}
</script>
