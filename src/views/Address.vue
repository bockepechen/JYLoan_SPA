      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressListFilter" @click="addressChecked=index;selectedAddressId=item.addressId" v-bind:class="{'check':addressChecked == index}">
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showMore()" v-bind:class="{'open':limitNum > 3}">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            <a href="javascript:;" class="btn btn--m btn--red" @click="order()">Next</a>
      ...
      methods:{
          getAddressList() {
              axios.get('/users/addressList').then((res)=>{
                  ...
                  if (recData.status == '0') {
                    this.addressList = recData.result;
          ...
          setDefault(item) {
              axios.post('/users/setDefault',{
                ...
                let recData = res.data;
                if (recData.status == '0') {
                    console.log("Set dafault address is success.");
                    this.getAddressList();
                } else {
                    console.log("Set dafault address is fail.");
                }
              });
          },
          delAddressConfirm(addressId) {
              this.addressId4Del = ''
              this.addressId4Del = addressId;
              this.mdShowDel = true;
          },
          delAddress() {
              axios.post('/users/delAddress',{
                'addressId':this.addressId4Del
              }).then((res)=>{
                let recData = res.data;
                this.mdShowDel = false;
                if (recData.status == '0') {
                  console.log("Delete address is success.");
                  this.getAddressList();
                } else {
                  console.log("Delete address is fail.");
                }
              });
          },
          closeDelModal() {
