<template>
  <div>
              ...
              <li v-for="(item,index) in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascript:;" class="checkbox-btn item-check-btn " v-bind:class="{'checked':item.checked=='1'}" @click="editProductNum('checked',item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editProductNum('sub',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="editProductNum('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascript:;" @click="allChecked()">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{totalAmount | currency('￥')}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" v-bind:class="{'btn--dis':checkedNum == 0}" @click="checkOut()">
                  Checkout
                </a>
              </div>
            </div>
    ...
    <!-- 调用模态窗口（删除商品） -->
    <modal-window v-bind:mdShow="mdShowDel" v-on:close="closeDelModal()">
      <p slot="contentSlot">
        <span>确认要删除这件商品吗？</span>
      </p>
      <div slot="buttonSlot">
        <a href="javascript:;" class="btn btn--m" @click="delCart()">确定</a>
        <a href="javascript:;" class="btn btn--m btn--red" @click="closeDelModal()">关闭</a>
      </div>
    </modal-window>
    <!-- 调用Footer组件 -->
    ...
    export default{
        ...
        computed:{
          ...
          totalAmount () {
              ...
              this.cartList.forEach((item)=>{
                if (item.checked == '1' ) {
                  totalAmount += parseFloat(item.salePrice)*item.productNum;
                }
              });
              return totalAmount;
          }
      ...
      methods: {
          getCartList() {
              axios.get('/users/cartList').then((res)=>{
                  ...
                  if (recData.status == '0') {
                      this.cartList = recData.result;
                  } else if (recData.status = '101') {
                    // 未登录访问购物车，被拦截之后转向首页（原生js 跳转方法）
          ...
          delCartConfirm(productItem) {
              this.productItem4Del = productItem;
              this.mdShowDel = true;
          },
          closeDelModal() {
          ...
          delCart() {
              axios.post('/users/delCart',{
                productId:this.productItem4Del.productId
              }).then((res)=>{
                  let recData = res.data;
                  if (recData.status = '0') {
                      console.log("Delete is success.");
                      this.mdShowDel = false;
                      this.$store.commit('updateCartNum', -this.productItem4Del.productNum);
                      this.getCartList();
                  } else {
...
//              if (item.checked == 1) {
//                this.$store.commit('updateCartNum', parseInt(item.productNum));
//              } else {
//                this.$store.commit('updateCartNum', -parseInt(item.productNum));
//              }
            }
            // 更新商品数量
            axios.post('/users/editProductNum',{
              ...
              checked:item.checked
                ...
                let recData = res.data;
                if (recData.status = '0') {
                    console.log('update num is succcess.')
                } else {
                  console.log('update num is fail.')
                }
            });
          },
          // 全选按钮事件
          allChecked() {
              // computed中的实时计算变量【checkedAllFlag】不能直接取反更改，需要中间变量
              var temp = this.checkedAllFlag;
              var cartNum = 0;
              this.cartList.forEach((item)=>{
                  item.checked = temp ? '0':'1';  // 循环购物车数据，根据checkedAllFlag，进行取反
                  cartNum += parseInt(item.productNum);
              });
//              if (temp) {
//                  this.$store.commit('resetCartNum', 0);
//              } else {
//                  this.$store.commit('resetCartNum', cartNum);
//              }
              // 更新商品选中状态
              axios.post('/users/checkedAll', {
                checkedAllFlag:this.checkedAllFlag
                  ...
                  let recData = res.data;
                  if (recData.status = '0') {
                      console.log("save allcheck is success.");
                  } else {
                      console.log("save allcheck is fail");
                  }
              });
          },
          // 结账事件
